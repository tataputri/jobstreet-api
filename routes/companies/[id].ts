...
export default async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!req.params.id) {
    return res.json({
      success: false,
      error: 'Please provide compay id'
    })
  }
  ...
  try {
    ...
    const response = await axios.get(`https://api-js.prod.companyreview.co/companies/${companyId}`, {
      headers: {
    ...
    const foreignId = response.data.data.foreign_company_id
    const currentJobs = await axios.get(`https://api-js.prod.companyreview.co/jobs/${foreignId}`, {
      params: {
        'page': '1',
        'language': 'en',
        'country': 'id'
